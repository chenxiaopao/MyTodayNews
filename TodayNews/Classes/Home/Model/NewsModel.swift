...
//    let emojiManager = EmojiManager()
    ...
    var titleH: CGFloat {
        ...
        if video_duration != 0 && video_style == 0 { // // 右侧有图
            return title.calculateHeightWithString(fontSize: UIFont.systemFont(ofSize: 17), width: kScreenW * 0.72 - 30)
        } else {
           return title.calculateHeightWithString(fontSize: UIFont.systemFont(ofSize: 17), width: kScreenW  - 30)
        }
    }
    var cellHeight: CGFloat {
        // 15 + titleH + 10 + middleViewH + 16 + 10 + bottomViewH + 10
        var height: CGFloat = 71 + titleH
        ...
        if middle_image.url != "" && image_list.count == 0 {
            ...
            if image_list.count != 0 {
                if image_list.count == 1 { return 140 }
                else { height += image3Width-10}
            }
        }
    ...
    var weitoutiaoHeight: CGFloat {
        ...
        var height: CGFloat = 127
        height += contentH
        height += collectionViewH
        return height
    }
    var ppmmCellHeight: CGFloat { return contentH + 75 + kScreenW * 1.4 }
    let emojiManager = EmojiManager()
    ...
    var contentH: CGFloat { return content.calculateHeightWithString(fontSize: UIFont.systemFont(ofSize: 16), width: kScreenW - 30) }
    var collectionViewH: CGFloat {
        return NewsModel.collectionViewHeight(thumb_image_list.count)
    }
    /// 计算高度
